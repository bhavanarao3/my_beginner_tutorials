...
#include <ctime>  // Include <ctime> for time functionality
#include <memory>
#include <string>
 ...
 * to toggle the base output string.
 *
 * This class demonstrates a basic publisher that sends messages containing the
 * current time to a topic named `custom_topic`. It also provides a service to
 * toggle the base output string, allowing dynamic changes in the message
 * format. The node also handles various logging levels based on user-defined
 * parameters.
 */
class MinimalPublisher : public rclcpp::Node {
 public:
   ...
   *
   * Initializes the publisher, service, timer, and parameters for publish
   * frequency, frequency threshold, and log level. The node's constructor sets
   * up the necessary components for publishing messages and handling service
   * requests.
   */
  MinimalPublisher()
      : Node("minimal_publisher"),
        base_output_("Current time: "),
        publish_frequency_(500) {
    ...
    service_ = this->create_service<example_interfaces::srv::SetBool>(
        "toggle_base_output",
        std::bind(&MinimalPublisher::ChangeBaseOutputCallback, this,
 ...
 private:
   ...
   *
   * This function is called periodically based on the specified publish
   * frequency. It publishes the current time to the topic `custom_topic`. It
   * also handles various log levels and conditions like no active subscribers,
   * errors if the publish frequency is too low, and fatal conditions for very
   * low publish frequencies.
   */
  void TimerCallback() {
    // Check the number of active subscribers before publishing
    if (publisher_->get_subscription_count() == 0) {
      RCLCPP_WARN(this->get_logger(),
                  "No active subscribers on 'custom_topic'.");
    }
    ...
    if (publisher_->get_subscription_count() == 0) {
      if (log_level_ <= 2) {
        RCLCPP_WARN(this->get_logger(),
                    "No subscribers, publishing to topic may not be useful.");
      }
    }
    ...
    message.data =
        base_output_ + std::string(std::ctime(&now));  // Convert to string
    ...
    if (publish_frequency_ < frequency_threshold_) {
      if (log_level_ <= 3) {
        RCLCPP_ERROR(this->get_logger(),
                     "Publish frequency is below the threshold! Frequency: %d",
                     publish_frequency_);
      }
    }

    // Simulate fatal condition if a critical condition is met (e.g., frequency
    // is too low)
    if (publish_frequency_ < 100) {
      if (log_level_ <= 4) {
        RCLCPP_FATAL(this->get_logger(),
                     "Fatal error: Publish frequency is too low! Terminating.");
    ...
    if (log_level_ <= 0) {
      RCLCPP_DEBUG(this->get_logger(), "Debug: Message published at %s",
                   message.data.c_str());
    }
  }
   ...
   *
   * This service allows clients to toggle the base output string that is used
   * in the published messages. If the request data is `true`, the base output
   * is set to "Custom time: ". Otherwise, it is set to "Current time: ".
   *
  ...
  void ChangeBaseOutputCallback(
    ...
    if (log_level_ <= 1) {
      RCLCPP_INFO(this->get_logger(), "Base output string changed to: '%s'",
                  base_output_.c_str());
    }
  }
  ...
  // Log level to control verbosity (0: DEBUG, 1: INFO, 2: WARN, 3: ERROR, 4:
  // FATAL)
  int log_level_;
};
 ...
 *
 * Initializes the ROS2 system and runs the `MinimalPublisher` node. The node
 * will continue spinning until it is manually shutdown.
 *
