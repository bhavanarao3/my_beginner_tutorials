 ...
 *
 * This test suite checks the functionality of the Talker node by verifying that:
 * 1. The publisher is correctly sending messages on the "/chatter" topic.
 * 2. The node is correctly broadcasting a transform from the "world" frame to the "talk" frame.
 */
TEST_CASE("Talker node integration test", "[talker]") {
   ...
   *
   * This test subscribes to the "/chatter" topic and verifies that the published
   * message contains the string "Current time".
   */
  ...
  /**
   * @brief Test case for checking the transform broadcast from the "talk" frame to the "world" frame.
   *
   * This test verifies that the Talker node broadcasts a transform from the "talk" frame to the
   * "world" frame with the correct translation and rotation values.
   */
